...
- **Points model trains one direct per-position regressor** on `total_points`
  ...
  the full reasoning. Phase 3 validation report (`data/reports/`, regenerated by
  `fplscout train`) shows this beats both required baselines (FPL's own `xP` and
  a naive 5-GW average) overall and per-position on RMSE, though GKP and FWD
  rank-correlation (Spearman) trail the naive baseline slightly — a concrete lead
  for the plan's post-Phase-3 model-error-analysis review, not something papered
  over here.
